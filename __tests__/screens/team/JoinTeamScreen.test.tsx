...
function renderScreen() {
  ...
  return { navigation, promise: renderWithProviders(
...
describe('<JoinTeamScreen />', () => {
  it('shows an inline error when the code is invalid or no longer redeemable', async () => {
    mockUseJoinTeam.mockReturnValue({ mutateAsync: jest.fn(), isPending: false });
    mockPreviewInviteCode.mockResolvedValueOnce(null);
    const { promise } = renderScreen();
    await promise;

    await fireEvent.changeText(screen.getByTestId('join-team-code'), 'BADCODE1');
    await fireEvent.press(screen.getByTestId('join-team-check'));

    await waitFor(() =>
      expect(screen.getByText("That code isn't valid or is no longer accepting players.")).toBeTruthy()
    );
  ...
  it('shows a confirm step naming the team before joining', async () => {
    ...
    await fireEvent.changeText(screen.getByTestId('join-team-code'), 'HJKM2345');
    await fireEvent.press(screen.getByTestId('join-team-check'));

    await waitFor(() => expect(screen.getByText('Join Riverside Rays?')).toBeTruthy());
  ...
  it('surfaces the RPC error message (e.g. a code revoked between preview and confirm)', async () => {
    mockUseTeam.mockReturnValue({ setActiveTeam: jest.fn() });
    const mutateAsync = jest.fn().mockRejectedValue(new Error('Invite code has been revoked'));
    mockUseJoinTeam.mockReturnValue({ mutateAsync, isPending: false });
    mockPreviewInviteCode.mockResolvedValueOnce({
      teamId: 'team-1',
      teamName: 'Riverside Rays',
    ...
    await waitFor(() => expect(screen.getByText('Invite code has been revoked')).toBeTruthy());
  });
});

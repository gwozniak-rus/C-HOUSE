...
beforeEach(() => {
  mockUseAuth.mockReturnValue({
    session: { user: { id: "user-coach" } } as Session,
  });
  ...
  mockUseSetMemberStatus.mockReturnValue({
    mutateAsync: jest.fn(),
    isPending: false,
  });
...
describe("<RosterScreen />", () => {
  it("shows a coach the mark-inactive and remove actions for each row", async () => {
    ...
    expect(
      screen.getByTestId("roster-row-user-player-toggle-status"),
    ).toBeTruthy();
  ...
  it("hides row actions from a player", async () => {
    ...
    expect(
      screen.queryByTestId("roster-row-user-player-toggle-status"),
    ).toBeNull();
  ...
  it("requires confirmation before removing a member, then removes on confirm", async () => {
    ...
    await waitFor(() =>
      expect(mutateAsync).toHaveBeenCalledWith("user-player"),
    );
  });
  ...
  it("keeps the confirm dialog open and shows the trigger message when removing the last coach fails", async () => {
    const mutateAsync = jest
      .fn()
      .mockRejectedValue(new Error("Cannot remove the last coach from a team"));
    mockUseRemoveMember.mockReturnValue({ mutateAsync, isPending: false });
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Cannot remove the last coach from a team"),
      ).toBeTruthy(),
    );

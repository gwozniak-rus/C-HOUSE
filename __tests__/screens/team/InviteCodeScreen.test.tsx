...
beforeEach(() => {
  mockUseTeam.mockReturnValue({
    activeTeam: { id: "team-1", name: "Riverside Rays" },
    activeTeamId: "team-1",
  });
  mockUseInviteCode.mockReturnValue({ data: activeCode, isPending: false });
  mockUseToggleJoining.mockReturnValue({
    mutateAsync: jest.fn(),
    isPending: false,
  });
  mockUseRegenerateCode.mockReturnValue({
    mutateAsync: jest.fn(),
    isPending: false,
  });
  jest
...
describe("<InviteCodeScreen />", () => {
  it("shows the current code and reflects an active code as accepting players", async () => {
    ...
    expect(screen.getByTestId("invite-code-value")).toHaveTextContent(
      "HJKM2345",
    );
  ...
  it("copies the code to the clipboard", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("invite-code-copied")).toBeTruthy(),
    );
  });
  ...
  it("toggles joining off, calling the mutation with the code id", async () => {
    ...
    await renderWithProviders(<InviteCodeScreen />);
    await fireEvent(
      screen.getByTestId("invite-code-toggle"),
      "valueChange",
      false,
    );
  ...
  it("requires confirmation before regenerating, then regenerates with the previous code id", async () => {
    ...
    await fireEvent.press(
      screen.getByTestId("invite-code-regenerate-confirm-confirm"),
    );

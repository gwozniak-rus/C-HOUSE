...
const setSession = (require('../../../lib/auth-context') as { __setSession: (s: unknown) => void })
  .__setSession;
...
describe('usePushNotifications', () => {
  ...
  it('reports unsupported and skips setup work when push is not supported', async () => {
    ...
    const { result } = await renderHook(() => usePushNotifications(), { wrapper: AuthProvider });
  ...
  it('registers the service worker and reflects an existing subscription', async () => {
    ...
    mockGetExistingSubscription.mockResolvedValue({ endpoint: 'https://push.example/existing' });
  ...
  it('subscribe() sets an error instead of calling subscribeToPush() when signed out', async () => {
    ...
    expect(result.current.error).toBe('You must be signed in to enable notifications');
  });

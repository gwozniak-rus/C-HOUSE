...
beforeEach(() => {
  ...
  ({ persistSubscription: mockPersistSubscription } = require('../../../lib/push/subscription'));
});
...
describe('registerServiceWorker', () => {
  ...
  it('resolves to null and logs when registration rejects', async () => {
    ...
    env.serviceWorker.register.mockRejectedValueOnce(new Error('registration failed'));
  ...
  it('persists a rotated subscription when the service worker posts one', async () => {
    const session = { user: { id: 'user-1' } } as unknown as import('@supabase/supabase-js').Session;
    ...
    await handler({ data: { type: 'PUSH_SUBSCRIPTION_CHANGED', subscription } });

    expect(mockPersistSubscription).toHaveBeenCalledWith('user-1', subscription);
  });
  ...
  it('does not persist a rotated subscription when there is no signed-in user', async () => {
    ...
    await handler({
      data: { type: 'PUSH_SUBSCRIPTION_CHANGED', subscription: { endpoint: 'x' } },
    });

...
describe('persistSubscription', () => {
  ...
  it('throws when the lookup query errors', async () => {
    ...
    const lookupBuilder = {
      select: jest.fn().mockReturnThis(),
      eq: jest.fn().mockReturnThis(),
      maybeSingle: jest.fn().mockResolvedValue({ data: null, error: lookupError }),
    };
    mockSupabase.from.mockReturnValueOnce(lookupBuilder as never);

    await expect(persistSubscription('user-1', validSubscriptionJson)).rejects.toBe(lookupError);
  });
  ...
  it('throws when the insert errors', async () => {
    ...
    mockSupabase.from
      .mockReturnValueOnce(lookupBuilder as never)
      .mockReturnValueOnce({ insert: jest.fn().mockResolvedValue({ error: insertError }) } as never);

    await expect(persistSubscription('user-1', validSubscriptionJson)).rejects.toBe(insertError);
  });
});
...
describe('unsubscribeFromPush', () => {
  ...
  it('unsubscribes from the browser and marks the row inactive', async () => {
    ...
    expect(eqAfterUpdate).toHaveBeenCalledWith('endpoint', 'https://push.example/abc');
  ...
  it('throws when marking the row inactive fails', async () => {
    ...
    mockSupabase.from.mockReturnValueOnce({
      update: jest.fn().mockReturnValue({
        eq: jest.fn().mockReturnValue({ eq: jest.fn().mockResolvedValue({ error: updateError }) }),
      }),
    } as never);

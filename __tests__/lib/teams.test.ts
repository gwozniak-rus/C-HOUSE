...
describe('mapSupabaseError', () => {
  it('passes through the known redeem_invite_code messages verbatim', () => {
    ...
    expect(mapSupabaseError({ message: 'Invalid invite code' })).toBe('Invalid invite code');
  });

  it('passes through the last-coach protection message', () => {
    expect(mapSupabaseError(new Error('Cannot remove the last coach from a team'))).toBe(
  ...
  it('replaces an unrecognized error with the generic fallback, never leaking raw Postgres text', () => {
    expect(mapSupabaseError(new Error('duplicate key value violates unique constraint'))).toBe(
      'Something went wrong. Please try again.'
    );
  });

  it('uses a caller-supplied fallback message when given one', () => {
    expect(mapSupabaseError(new Error('weird internal error'), 'Could not save.')).toBe(
      'Could not save.'
    );
  });
});
...
describe('createTeam', () => {
  it('inserts with created_by and returns the created row', async () => {
    ...
    const result = await createTeam({ name: '  Riverside Rays  ', createdBy: 'user-1' });
  ...
  it('throws the raw error so mapSupabaseError can classify it upstream', async () => {
    ...
    await expect(createTeam({ name: 'x', createdBy: 'user-1' })).rejects.toBe(error);
  });
});
...
describe('getRoster', () => {
  it('sorts coaches before players, then alphabetically by last name', async () => {
    const rows = [
      {
        ...
        profiles: { first_name: 'Zed', last_name: 'Beta', display_name: 'Zed Beta' },
      },
      {
        user_id: 'u-coach',
        role: 'coach',
        status: 'active',
        joined_at: '2026-01-01',
        profiles: { first_name: 'Dana', last_name: 'Whitfield', display_name: 'Dana Whitfield' },
      },
      {
        user_id: 'u-player-a',
        role: 'player',
        status: 'active',
        joined_at: '2026-01-01',
        profiles: { first_name: 'Marcus', last_name: 'Alpha', display_name: 'Marcus Alpha' },
      },
    ];
    ...
    expect(roster.map((m) => m.userId)).toEqual(['u-coach', 'u-player-a', 'u-player-b']);
  });
});
...
describe('previewInviteCode / redeemInviteCode', () => {
  ...
  it('normalizes the code before calling the RPC and maps the returned row', async () => {
    mockSupabase.rpc.mockResolvedValueOnce({
      data: [{ team_id: 'team-1', team_name: 'Riverside Rays', role: 'player' }],
      error: null,
    });
    ...
    expect(result).toEqual({ teamId: 'team-1', teamName: 'Riverside Rays', role: 'player' });
  });
...
describe('regenerateInviteCode', () => {
  it('creates the new code before revoking the previous one, so the team is never codeless', async () => {
    ...
    const insertSingle = jest.fn().mockResolvedValue({ data: { id: 'code-2', code: 'NEW1' }, error: null });
    ...
    mockSupabase.from.mockReturnValueOnce({ insert } as never).mockReturnValueOnce({ update } as never);
  ...
  it('skips revoking when there was no previous code', async () => {
    const insertSingle = jest.fn().mockResolvedValue({ data: { id: 'code-1', code: 'NEW1' }, error: null });
    const insertSelect = jest.fn().mockReturnValue({ single: insertSingle });

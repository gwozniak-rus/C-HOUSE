...
function baseState(overrides: Partial<ReturnType<typeof usePushNotifications>> = {}) {
  return {
    isSupported: true,
...
describe('<PushNotificationToggle />', () => {
  ...
  it('shows a blocked message when permission is denied', async () => {
    mockUsePushNotifications.mockReturnValue(baseState({ permission: 'denied' }));
  ...
  it('shows a "Disable notifications" button when subscribed and calls unsubscribe() on press', async () => {
    ...
    mockUsePushNotifications.mockReturnValue(baseState({ isSubscribed: true, unsubscribe }));
  ...
  it('renders the error message when present', async () => {
    mockUsePushNotifications.mockReturnValue(baseState({ error: 'Something went wrong' }));
